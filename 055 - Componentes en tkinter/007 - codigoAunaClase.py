...
class ComponentesVentana(tk.Tk):
  def __init__(self):
    super().__init__()
    self.geometry('640x420+640+280')
    self.title('Componentes')
    self.iconbitmap('icono.ico')
    self._crear_tabs()

  def _crear_componentes_tabulador(self, tabulador):
  ...
  def _crear_componentes_tabulador3(self, tabulador):
    # Crar una lista usando data list comprenhensions
    ...
    def mostrar_valor():
      messagebox.showinfo(f'Valor seleccionado', f'valor seleccionado: {combobox.get()}')
    boton1 = ttk.Button(tabulador, text='Mostrar valor', command=mostrar_valor)
  ...
  def _crear_componentes_tabulador4(self, tabulador):
    ...
    boton_imangen = ttk.Button(tabulador, image=imagen, command=mostrar_titulo)
    boton_imangen.grid(row=0, column=0)
  ...
  def _crear_componentes_tabulador5(self, tabulador):
    ...
    # Agregamos los métodos para controlar los eventos de nuestra barra de progreso
    def ejecutar_barra():
  ...
  def _crear_tabs(self):
    ...
    self._crear_componentes_tabulador(tabulador1)
...
if __name__ == '__main__':
  componente_ventana1 = ComponentesVentana()
  componente_ventana1.mainloop()