...
class Editor(tk.Tk):
  def __init__(self):
    ...
    # Configuración mínima segunda columna
  ...
  def _guardar_como(self):
    ...
    # Leemos el contenido de la caja de texto
    texto = self.campo_texto.get(1.0, tk.END) 
    # Escribimos en contenido al nuevo archivo
    self.archivo.write(texto)
    # Cambiamos el nombre del archivo
    self.title(f'Editor Texto - {self.archivo.name}')
    # cerramos el archivo
    self.archivo.close()
    # Indicamos que ya hemos abierto una archivo
    self.archivo_abierto = self.archivo.name
